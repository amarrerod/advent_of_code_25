...
function is_accessible(roll: string, rolls: Set<string>, rows: number, cols: number): boolean {
...
function remove_amap(rolls: Set<string>, rows: number, cols: number): number {
    ...
    let removed = 0;
    while (to_remove > 0) {
        const rolls_to_remove = [...rolls].filter(r => is_accessible(r, rolls, rows, cols));
